...
object BitmapUtils {
    ...
    fun getBitmapFromGallery(context: Context, path:Uri, width: Int, height: Int) :Bitmap{
        val fillePathColumn = arrayOf(MediaStore.Images.Media.DATA)
        val cursor = context.contentResolver.query(path, fillePathColumn, null, null, null)
        cursor!!.moveToFirst()
        val columnIndex = cursor.getColumnIndex(fillePathColumn[0])
    ...
    private fun calculateInSampleSize(options: BitmapFactory.Options, regWidth: Int, regHeight: Int): Int {
        ...
        if(height > regHeight || width > regWidth)
        {
            ...
            while (hafHeight / inSampleSize >= regHeight && hafWidth / inSampleSize >= regWidth)
                inSampleSize *= 2
        }
        return inSampleSize
    }
